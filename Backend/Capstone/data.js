...
Spreadsheet.ComputeAlgorithmOnFile = async function(filename){
    ...
    for (var Number_Of_Courses = 0;  Number_Of_Courses < Total_Courses; Number_Of_Courses++)
    {
        ...
        for(var R = range.s.r+1; R <= range.e.r; ++R) {
            ...
            var First_Name = Course_Name.getCell(XLSX.utils.encode_cell({c:3, r:R})).value
            var Last_Name = Course_Name.getCell(XLSX.utils.encode_cell({c:4, r:R})).value
            var Professor_Choice = Course_Name.getCell(XLSX.utils.encode_cell({c:5, r:R})).value
            var TA_Choice = Course_Name.getCell(XLSX.utils.encode_cell({c:6, r:R})).value
            var Lab_Instructor_Choice = Course_Name.getCell(XLSX.utils.encode_cell({c:7, r:R})).value
            var Taught_before = Course_Name.getCell(XLSX.utils.encode_cell({c:8, r:R})).value
            var Taken_Course = Course_Name.getCell(XLSX.utils.encode_cell({c:9, r:R})).value
            var TA_Major_Matches = Course_Name.getCell(XLSX.utils.encode_cell({c:10, r:R})).value

            var Choice = new Choices(First_Name,Last_Name,Professor_Choice,TA_Choice,Lab_Instructor_Choice,Taught_before,Taken_Course,TA_Major_Matches);
